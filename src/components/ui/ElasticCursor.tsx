...
function ElasticCursor() {
  ...
  useLayoutEffect(() => {
    ...
    if (!isMobile) window.addEventListener("mousemove", setFromEvent);
    return () => {
      if (!isMobile) window.removeEventListener("mousemove", setFromEvent);
    };
  }, [isMobile, loop]);
  ...
  useEffect(() => {
    ...
    jellyRef.current.style.width = "3.125rem";
  }, []);
